 ...
 * 断言 {@link StructureRegistry} 全名单 = <b>39</b> 名（26 城 + 6 outpost + 5 机型 + 2 husk）
 * 且维度归属 PROSPERITY=37 / SHATTERED=2，并逐名打印。
 * <p>
...
public class S8RegistryRosterCheck {
    ...
    /** 期望总名数（P0 起钉为 39；旧值 33 少 6 outpost，属 stale）。 */
    static final int EXPECTED_TOTAL = 39;

    /** 39 名全名单（与实机日志 plan/log.txt:11452 同源；有序，便于 diff 定位）。 */
    ...
    public static void main(String[] args) {
        ...
        // ① 分组计数（旧断言把 outpost 混进 cities 桶且总数钉 33 ⇒ stale）
        final int sourceCounts = CityVariants.ALL.length + RuinedMachineShapes.ALL.length
            + ProsperityOutpostPlacer.ALL.length;
        if (names.size() != EXPECTED_TOTAL || machines != 5 || cities != 26 || outposts != 6 || husks != 2
            || CityVariants.ALL.length != 26 || RuinedMachineShapes.ALL.length != 5
            || ProsperityOutpostPlacer.ALL.length != 6) {
            System.out
                .println("ROSTER FAIL: total=" + names.size() + " machines=" + machines + " cities=" + cities
                    + " outposts=" + outposts + " husks=" + husks
                    + " cityAll=" + CityVariants.ALL.length + " machineAll=" + RuinedMachineShapes.ALL.length
                    + " outpostAll=" + ProsperityOutpostPlacer.ALL.length + " sourceCounts=" + sourceCounts);
            System.out.println("NAMES: " + names);
            System.exit(1);
        }
        // ② 名集合与 39 名期望全名单相等（不只数量相等；逐名 footprint + 模板 SHA 在 RosterIntegrityCheck）
        final Set<String> missing = new TreeSet<>(Arrays.asList(EXPECTED_NAMES));
        missing.removeAll(new LinkedHashSet<>(names));
        final Set<String> extra = new TreeSet<>(names);
        extra.removeAll(Arrays.asList(EXPECTED_NAMES));
        if (!missing.isEmpty() || !extra.isEmpty()) {
            System.out.println("ROSTER FAIL: name set mismatch missing=" + missing + " extra=" + extra);
            System.out.println("NAMES: " + names);
            System.exit(1);
        }
        // ③ 6 outpost 必须逐个在册且归 PROSPERITY
        ...
        System.out.println("ROSTER PASS: StructureRegistry names() = 39 (5 machines + 26 city + 6 outpost + 2 husk)");
        ...
        System.out.println("DIM78 registry names = " + (names.size() - husks) + " (machines=" + machines
            + " outposts=" + outposts + " cities=" + cities + ")，与实机日志 plan/log.txt:11452 口径一致");
        System.out.println("NAMES(39): " + names);

 ...
 * <p>
 * <b>纯 JEP 330 单文件运行，零 Minecraft 执行依赖</b>（城/outpost 腿零 MC；husk 腿反射已编译类，
 ...
 * 'C' 一字两义（city/outpost = 圆石；husk = 核心位积碳壳 meta1），图例条目带 groups 域区分；</li>
...
public class StructureViewerExport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("EXPORT variants=34 (city=26 outpost=6 husk=2) bytes=" + first.getBytes("UTF-8").length);
    ...
    String buildJson() throws Exception {
        ...
        b.append("\n  ],\n");
        b.append("  \"groups\": [\n");
        b.append("    {\"id\":\"city\",\"name\":\"繁荣废都·城变体（dim78）\",\"source\":\"CityVariants.ALL\",\"count\":26},\n");
        b.append("    {\"id\":\"outpost\",\"name\":\"城外中型废墟（dim78 outposts）\",\"source\":\"ProsperityOutpostPlacer.ALL\",\"count\":6},\n");
        b.append("    {\"id\":\"husk\",\"name\":\"破碎残骸骨架（dim79）\",\"source\":\"WorldGenShatteredRuins（反射形状）\",\"count\":2}\n");
        b.append("  ],\n");
        b.append("  \"variants\": [\n");
    ...
    /** 调色板图例：char → 颜色 → 语义名 → meta 备注；'C' 一字两义以 groups 域区分。 */
    private void appendPalette(StringBuilder b) {
        final String[][] legend = {
            { "#", "gtsr:RuinedCasing", "0", "锈壳（外壳族 meta0）", "city,outpost,husk", "husk 语境 = 积碳外层（WorldGenShatteredRuins 口径）" },
            { "@", "gtsr:RuinedCasing", "1", "积碳壳（外壳族 meta1）", "city,outpost", "烟黑" },
            { "%", "gtsr:RuinedCasing", "2", "碎瓷壳（外壳族 meta2）", "city,outpost", "青白" },
            ...
            { "C", "minecraft:cobblestone", "0", "圆石", "city,outpost", "husk 的 'C' 另有条目（核心位积碳壳）" },
    ...
    /** 与 appendPalette 平行的 (groups, char) 表（仅用于断言面）。 */
    private static final String[][] PALETTE_GROUPS = { { "city,outpost,husk", "#" }, { "city,outpost", "@" },
        { "city,outpost", "%" }, { "city,outpost", "d" }, { "city,outpost", "p" }, { "city,outpost", "r" },
        { "city,outpost", "c" }, { "city,outpost", "s" }, { "city,outpost", "S" }, { "city,outpost", "C" },
        { "city,outpost", "G" }, { "city,outpost", "B" }, { "city,outpost", "X" }, { "city,outpost", "Z" },
        { "husk", "C" } };

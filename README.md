...
- **蒸汽奇点压缩机 / Steam Singularity Compressor (SSC)**: 2-tier heat-based machine. Devours the highest-grade steam in the input hatches (Tier 1: Steam/Superheated/Supercritical; Tier 2: also dense variants) without any EU cost, raising heat via logarithmic equations. At 100% heat it produces 1 Steam Entangled Singularity (Tier 1) or 1 Critical Steam Entangled Singularity (Tier 2). Tier 2 screwdriver cycles three modes (machine off only): Accumulate / Steam Compression (1000:1 steam → dense) / Steam Decompression (1:1000 dense → steam); entering Compression/Decompression auto-clears heat, each singularity fuels 600s, same-type steam accumulates fractionally (different type resets it). Heat decays 1%/s whenever the machine is off (including soft-hammer shutdown) or starved. A marvel of the steam age: compressing massive amounts of steam, breaking through spatial limitations, creating cross-dimensional connections. No parallel.
  双等级热量机制：吞噬输入仓中最高等级的蒸汽（等级1：蒸汽/过热/超临界；等级2：另含全部致密变体），无电力消耗，按对数方程提升热量；热量达到 100% 时产出 1 个蒸汽纠缠奇点（等级1）或 1 个临界蒸汽纠缠奇点（等级2）。等级2 螺丝刀循环切换三模式（仅需关机）：蓄热 / 蒸汽压缩（1000:1 蒸汽→致密）/ 蒸汽解压（1:1000 致密→蒸汽）；切入压缩/解压自动清空热量，每颗奇点续航 600s，同种蒸汽浮点累计、异种顶掉。机器关机（含软锤关闭）或无蒸汽时热量每秒 -1%。蒸汽时代的奇迹造物：压缩巨量的蒸汽，突破空间的限制，创造出跨越维度的连接。无并行。

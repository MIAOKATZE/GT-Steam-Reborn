...
/**
 * 破碎之地（dim79）三群系持有者 + 注册入口（dim1 S6a，plan §S6a / 04 §1.1）。
 * <p>
 * 职责：
 * <ol>
 * <li>preInit（BlockLoader 之后，群系 top/filler 引用的 BlocksGTSR.shattered* 已注册）调用
 * {@link #init}：按 {@link Config#shatteredBiomeIdStart}（默认 190..192）逐个经
 * {@link GTSRBiomeBase#isBiomeIdFree} 校验后再构造（构造即占 biomeList 槽且不可回退），
 * 被占群系跳过 + 告警，维度以剩余群系降级运行（plan R3，ProsperityBiomes 同款）；</li>
 * <li>把 3 群系按 04 §1.1 权重表 50/30/20 挂入 def（{@link GTSRDimensionDef#addBiome}），
 * 由 GTSRWorldChunkManager 展开为 per-chunk 选择表。</li>
 * </ol>
 * BiomeDictionary（END+DEAD+SPOOKY）在各群系构造器内自持；不调 addSpawnBiome（维度专属群系）。
 * 成功日志锚点（plan §6.1 grep 点，三槽全注册时）：{@code [GTSR] shattered biomes: 190..192 registered}。
 */
public final class ShatteredBiomes {

    /** 本维度群系槽数（biomeIdStart .. biomeIdStart+2）。 */
    private static final int BIOME_SLOT_COUNT = 3;

    private ShatteredBiomes() {}

    /**
     * S6a 注册入口：构造三群系（先空闲校验后构造）并挂入 def 权重表。
    ...
    public static void init(GTSRDimensionDef def) {
        ...
        int registered = 0;
        registered += attachBiome(def, start + 0, BiomeShatteredIsles.WEIGHT, BiomeShatteredIsles::new, ids);
        registered += attachBiome(def, start + 1, BiomeRiftCanyon.WEIGHT, BiomeRiftCanyon::new, ids);
        registered += attachBiome(def, start + 2, BiomeSingularityWaste.WEIGHT, BiomeSingularityWaste::new, ids);
        // 三槽全注册且连续时输出区间锚点（plan §S6a 验收 grep 串：shattered biomes: 190..192 registered）
    ...
    /** 天空/雾色按群系取色（04 §1.1 暗紫三变体；非本维群系回退浮岛色）。 */
    public static int skyFogColorFor(BiomeGenBase biome) {
        if (biome instanceof BiomeShatteredIsles) {
            return BiomeShatteredIsles.SKY_FOG_COLOR;
        }
        if (biome instanceof BiomeRiftCanyon) {
            return BiomeRiftCanyon.SKY_FOG_COLOR;
        }
        if (biome instanceof BiomeSingularityWaste) {
            return BiomeSingularityWaste.SKY_FOG_COLOR;
        }
        return BiomeShatteredIsles.SKY_FOG_COLOR;
    }
}

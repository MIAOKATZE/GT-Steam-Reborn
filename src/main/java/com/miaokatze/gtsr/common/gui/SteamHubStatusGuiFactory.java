...
public class SteamHubStatusGuiFactory extends AbstractUIFactory<PosGuiData> {
    ...
    public @Nonnull IGuiHolder<PosGuiData> getGuiHolder(PosGuiData data) {
        ...
        if (te instanceof IGregTechTileEntity gte && gte.getMetaTileEntity() instanceof MTESteamHubArray hub) {
            return new MTESteamHubStatusGui(hub);
        }
        throw new IllegalStateException(
            String
                .format("TileEntity at (%s, %s, %s) is not a Steam Hub Array!", data.getX(), data.getY(), data.getZ()));
    }

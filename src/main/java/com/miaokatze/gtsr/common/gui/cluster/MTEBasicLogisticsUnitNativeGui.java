...
public class MTEBasicLogisticsUnitNativeGui extends MTEClusterUnitNativeGui {
    ...
    protected ListWidget<IWidget, ?> createTerminalTextWidget(PanelSyncManager syncManager, ModularPanel parent) {
        ...
        list.child(IKey.dynamic(() -> {
            ...
            // 批冷却：就绪（绿）/ 剩余秒（黄）
            .child(IKey.dynamic(() -> {
                int cooldown = cooldownSync.getValue();
                if (cooldown <= 0) {
                    return EnumChatFormatting.YELLOW
                        + StatCollector.translateToLocal("gtsr.cluster.native.logistics.cooldown")
                        + EnumChatFormatting.GREEN
                        + StatCollector.translateToLocal("gtsr.cluster.native.logistics.cooldown.ready")
                        + EnumChatFormatting.RESET;
                }
                return EnumChatFormatting.YELLOW
                    + StatCollector.translateToLocal("gtsr.cluster.native.logistics.cooldown")
                    + EnumChatFormatting.YELLOW
                    + String.format("%ds", cooldown / 20)
                    + EnumChatFormatting.RESET;
            })
                .asWidget()
                .marginBottom(2)

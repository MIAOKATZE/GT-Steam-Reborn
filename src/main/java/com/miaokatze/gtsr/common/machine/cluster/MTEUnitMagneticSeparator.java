 ...
 *
 * <p>
 * 仅声明解锁的 ChainLink（磁选 MAGNETIC_SEPARATOR），自身零配方执行；配方匹配与执行由集群
 * 总控侧完成。纹理、集群接线与流体缓冲等公共行为全部继承自 MTEClusterUnitBase。
 *
 * <p>
 * 需集群能源仓持续供电（用户拍板，{@code ClusterParams.MAGNETIC_EU_PER_TICK}）：停电时本单元链路
 * 应视为不可用，通电闸门在总控侧判定（{@code cluster.isPoweredUnitActive}，总控切片提供），

...
public class MTEVoidCrustSteamBorerOLD extends MTESteamMultiBlockBase<MTEVoidCrustSteamBorerOLD>
    ...
    private static IStructureDefinition<MTEVoidCrustSteamBorerOLD> STRUCTURE_DEFINITION = null;
    ...
    public MTEVoidCrustSteamBorerOLD(int aID, String aName, String aNameRegional) {
        super(aID, aName, aNameRegional);
    }

    public MTEVoidCrustSteamBorerOLD(String aName) {
        super(aName);
    }
            ...
            if (!Loader.isModLoaded("gtneioreplugin")) {
                ...
                try {
                    itemDimDisplayClass = Class
                        .forName(ITEM_DIM_DISPLAY_CLASS, true, MTEVoidCrustSteamBorerOLD.class.getClassLoader());
    ...
    public IMetaTileEntity newMetaEntity(IGregTechTileEntity aTileEntity) {
        return new MTEVoidCrustSteamBorerOLD(mName);
    }
    ...
    protected void updateAllHatchTextures() {
        ...
        // v1.10.6：样板仓（mDualInputHatches）纹理更新（SteamInputBus 元素可接受样板仓）
    ...
    @Override
    public IStructureDefinition<MTEVoidCrustSteamBorerOLD> getStructureDefinition() {
        if (STRUCTURE_DEFINITION == null) {
            STRUCTURE_DEFINITION = StructureDefinition.<MTEVoidCrustSteamBorerOLD>builder()
                .addShape(
                ...
                .addElement(
                    ...
                    ofChain(
                        ...
                        onElementPass(MTEVoidCrustSteamBorerOLD::onCasingAdded, ofBlock(GregTechAPI.sBlockCasings2, 0)),
                        ...
                        buildHatchAdder(MTEVoidCrustSteamBorerOLD.class).atLeast(PressureSteamInputHatch)
                            .casingIndex(getCasingTextureID())
                            .hint(1)
                            .shouldReject(t -> !t.mSteamInputFluids.isEmpty() && !t.mInputHatches.isEmpty())
                            .build(),
                        buildHatchAdder(MTEVoidCrustSteamBorerOLD.class).atLeast(SteamInputBus, SteamOutputBus)
                            .casingIndex(getCasingTextureID())
                            .hint(1)
                            .build(),
                        // v1.9.40 新增：冷却仓元素（可选）。蒸汽消耗的冷却产物（普通→蒸馏水 160:1、
                        // 过热→蒸汽 1:1）由 mixin 推入对应冷却仓，此前结构无此元素导致产物滞留/丢失。
                        buildHatchAdder(MTEVoidCrustSteamBorerOLD.class)
                ...
                .addElement(
                    'C',
                    onElementPass(MTEVoidCrustSteamBorerOLD::onCasingAdded, ofBlock(GregTechAPI.sBlockCasings2, 13)))
                .addElement(
                    'D',
                    onElementPass(MTEVoidCrustSteamBorerOLD::onCasingAdded, ofBlock(GregTechAPI.sBlockCasings2, 3)))
                .addElement(
                    'E',
                    onElementPass(MTEVoidCrustSteamBorerOLD::onCasingAdded, ofBlock(GregTechAPI.sBlockCasings3, 14)))
                .addElement(
                    'F',
                    onElementPass(
                        MTEVoidCrustSteamBorerOLD::onCasingAdded,
    ...
    protected MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addInfo(EnumChatFormatting.RED + StatCollector.translateToLocal("gtsr.tooltip.old.remove_soon"))
            .addMachineType(StatCollector.translateToLocal("gtsr.tooltip.void_borer.type"))
